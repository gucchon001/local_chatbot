    ...
    for row in values[1:]:
        ...
        if source['参照形式'] == 'Webサイト':
            ...
            try:
                ...
                logger.info(f"ファイル用ディレクトリが作成されました: {source['persist_directory']}")
            except Exception as e:
                logger.error(f"ファイル用ディレクトリの作成に失敗しました: {e}")
                continue
        ...
        logger.info(f"  参照先の存在: {os.path.exists(source['参照先'])}")
        logger.info(f"  参照先のタイプ: {'ディレクトリ' if os.path.isdir(source['参照先']) else 'ファイル' if os.path.isfile(source['参照先']) else 'URL' if source['参照形式'] == 'Webサイト' else '不明'}")
